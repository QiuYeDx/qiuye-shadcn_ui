...
### DEMO-2：ASCII 与自定义演示输入增强

目标：让详情页完整 Demo 明确展示字符集默认值、动态 Source、多种形状和本地图片输入能力。
...
- 更新设计文档、feat 文档和实施记录，使能力边界与实际页面一致。

非范围：

- 把字符集或形状预设加入公共组件 API。
- 视频、摄像头、音频、远程 URL 输入、图片编辑或上传服务。
- 修改 Source/Renderer 核心契约或增加新的 npm 依赖。
...
- 创建 `MatrixEffect组件实施记录/<日期>_DEMO-2_ASCII与自定义演示输入增强.md`。
...
下一工作包为 `QA-2 视觉、响应式、性能与生命周期验收`。

开始前应读取 QA-1 实施记录并复用已通过的安装与功能基线，重点关闭 DPR 1/2/高 DPR 上限、横向/方形/纵向容器、多视口视觉、Chrome 4x/6x CPU 压力下的 30/60 FPS 降档与回升、离屏/后台暂停、运行时 Reduced Motion、反复挂载卸载资源清理，以及连续动画不触发每帧 React commit。继续使用 pnpm 8.7.0，不重复扩大公共 API。
