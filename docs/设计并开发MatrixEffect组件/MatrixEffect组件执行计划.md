...
下一工作包为 `FX-1 Dot Renderer 与 DotMatrixEffect`。

开始实现前应读取 FE-2 实施记录，复用单一 rAF、暂停恢复、有效播放时间和错误锁契约，实现 soft blob Source、Dot Renderer、`createCellRenderer()` 与 `DotMatrixEffect`。动态 Dot 主路径应同时验证 Renderer FPS 提示和生命周期门控；ASCII、Demo、站点接入和 registry 仍不属于 FX-1。
