...
三个权重必须是有限、非负数且总和大于 0；合法输入先按总和归一化，以确保标准 RGB 输入仍映射到 `[0, 1]`。非法权重回退到 Rec.709，并在开发环境只警告一次。`[1, 0, 0]`、`[0, 1, 0]`、`[0, 0, 1]` 可分别提取单一颜色通道。
...
- 使用 `React.useImperativeHandle()` 暴露稳定 handle。
