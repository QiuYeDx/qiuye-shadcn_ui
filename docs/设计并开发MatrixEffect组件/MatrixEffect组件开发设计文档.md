...
- 创建日期：2026-07-22
- 更新日期：2026-07-23（QA-1 完成后同步）
- 状态：QA-1 已完成，待启动 QA-2
- 组件 ID：`matrix-effect`
...
ASCII 默认: cellSize=10, cellAspectRatio=1, maxCells=6000
```
...
最终使用 100 列、自动推导 rows、`cellAspectRatio=1`、`maxCells=6000`，而不是回退到核心的 10000 cells。
...
默认使用固定色 `#71717a`、字重 `400`、`fontScale=1` 和跨平台等宽字体栈 `ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace`。`fontScale` 表示 `fontSize = cellHeight * fontScale`，非有限或小于等于 0 时回退 1。ASCII 的默认 `cellAspectRatio` 为 `1`，允许调用方覆盖。

`cellAspectRatio` 描述采样格和布局格的几何比例，不是单个 glyph 墨迹边界的宽高比。默认使用方形单元格，保证图片或程序化 Source 从采样 Canvas 映射到输出 Canvas 时 x/y 缩放一致，不会因等宽字体常见的窄字面而把整体图像横向压缩；glyph 继续使用字体自身比例并在方形单元格中居中。
...
- 字符集输入框初始值必须与组件默认字符集 `" .:-=+*#%@"` 完全一致，避免 Demo 默认值被误认为公共默认值。
- Demo 提供组件默认、精细层次、块元素和圆点四类字符集预设；预设只负责写入公共 `characters` 属性，不扩展组件枚举或改变自定义字符串、glyph 数组能力。
- 输入源可切换同源静态示例图片与多种 Demo 级程序化动态场，包括具有持续角相位推进的旋转星旋；使用 `fit="contain"` 保留静态图片和上传图片的完整主体。
- 动态场使用已有 `MatrixProceduralSource` 与核心单 rAF 调度，不在 Demo 内创建独立动画循环；提供播放开关并继续遵守 Reduced Motion、离屏暂停和 30 FPS 偏好。
- 支持用户上传本地图片并直接作为 `File` Source 使用；文件仅保留在当前页面状态，不上传、不持久化，也不创建 Demo 自己管理的 object URL。
- 可切换固定色/源图色、字符集、反相和采样密度；提供 Demo 级配色预设以及字符色、背景色取色器。
- 源图色模式下字符 RGB 来自 Source，字符色取色器保持当前值但禁用；切回固定色后继续使用该值。背景色在两种模式下始终可编辑。
- 视觉方向贴近参考图二，但不要求复制参考图片本身。

### 3. 自定义管线

- 使用 `MatrixEffect` + 自定义 Transform 或 `createCellRenderer()`。
- 与 ASCII 场景复用相同的输入源选择模型，支持静态图片、Demo 级动态场和本地图片上传。
- 示例应明显展示“同一 Source、更换映射/绘制逻辑即可得到新效果”，且动态 Source 在自定义 Renderer 下能够持续更新。
- 示例代码保持短小，避免把完整引擎逻辑复制到 Demo。

字符集、形状和配色预设属于站点演示资产，不进入 MatrixEffect 公共 API 或 Registry item。组件层继续只提供通用 `characters`、`colorMode`、`color`、`backgroundColor`、图片/Canvas/程序化 `source` 与 `playing` 契约，避免把演示审美固化为组件能力边界。

详情页快速预览使用 `DotMatrixEffect`，尺寸稳定且无需网络资源。完整 Demo 的容器必须设置稳定高度或 `aspect-ratio`，避免 Canvas 加载和动态内容造成布局跳动。

首页组件墙把 Matrix Effect 作为前排 `wide` 推荐项，同一卡片内并排展示圆点与 ASCII 两个效果。两者共享稳定的柔和流动光团 Source，但保持独立 Renderer 与 Canvas；颜色跟随页面深浅主题切换。首页预览固定使用 30 FPS、`maxDpr=1.5`、离屏暂停与受控网格上限，避免组件墙同时运行多个动画时产生不必要的性能开销。该预览只是站点展示层，不进入 Registry 安装产物。
...
- ASCII 默认使用 1:1 方形采样单元格；横、方、纵容器中的输入主体比例保持一致，字符字面在格内居中。
